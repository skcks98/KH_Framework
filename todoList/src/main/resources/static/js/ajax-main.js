console.log("ajxa-main.js loaded");
...
/*
  ...
  - Fulfiled (이행 됨) : 비동기 작업이 성공적으로 완료된 상태
...
function getCompleteCount(){ // 함수 정의
  ...
  fetch("/ajax/completeCount") // 기본 GET 요청
...
const selectTodoList = () =>{
  ...
  .then(todoList => {
    ...
    console.log(todoList); // JS 객체 배열 형태로 변환
...
//----------------------------------------------------------------------------------
// 수정 레이어에서 취소버튼(#updateCancel)이 클릭되었을 떄
...
// js 파일에 함수 호출 코드 바로 작성 -> 페이지 로딩시 바로 실행하여 화면에 출력하기 위해서.
