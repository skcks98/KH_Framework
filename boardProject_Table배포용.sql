...
TO_CHAR(ENROLL_DATE, 'YYYY"년" MM"월" DD"일" HH24"시" MI"1분" SS"초" ') ENROLL_DATE
FROM "MEMBER" 
...
(
	SELECT NEXT_IMG_NO(), '경로1', '원본1', '변경1', 1, 1993 FROM DUAL
	UNION
	SELECT NEXT_IMG_NO(), '경로2', '원본2', '변경2', 2, 1993 FROM DUAL
	UNION
	SELECT NEXT_IMG_NO(), '경로3', '원본3', '변경3', 3, 1993 FROM DUAL
);
...
SELECT CHATTING_ROOM_NO
	,(SELECT MESSAGE_CONTENT FROM (
		...
		WHERE ROWNUM = 1) LAST_MESSAGE					-- 각 채팅방에 가장 최근인것을 가져옴
