...
public class EditBoardController {
	 ...
	 * @throws Exception
	 */
	@PostMapping("{boardCode:[0-9]+}/insert") // 제목/상세내용 + 이미지들
	public String boardInsert(@PathVariable("boardCode") int boardCode,
		 ...
		 * [해결 방법]
		 * 무작정 서버에 저장 X
		 * -> List의 각 인덱스에 들어있는 MulripartFile에 실제로
	 ...
	 * @param model			: 포워드시 request scope로 값 전달하는 용도
	...
	public String boardUpdate(
			...
			) throws Exception{		// 파일을 다루기 때문에 throws Exception
		...
		if(result > 0) {
			...
			message = "수정 실패";
			path = "update";	// GET (수정 화면 전화) 리다이렉트하는 상대경로
		}
